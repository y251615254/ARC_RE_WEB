...
void set_ReDeviceName_cgi(int argc, char **argv, void *p);
...
//	{"apply_sipp.cgi",	TASK_t apply_sipp_cgi, 	TASK_STKSIZ},
	...
	{"set_redevicename.cgi",	TASK_t set_ReDeviceName_cgi, 	TASK_STKSIZ},
...
#if defined(WE410443_SA) || defined(WE410443_6DX)
...
#ifdef WE410443_6DX
		snprintf(buf, sizeof(buf), "Location: /warning.htm");
#else
...
void set_ReDeviceName_cgi(int argc, char **argv, void *p)
{
	...
	*/
	...
	if(!strncmp(cmd, "syncredevicename", 16))
	{
		...
		snprintf(jasonbuf, sizeof(jasonbuf),"{\"device_mac\":\"%s\", \"device_name\":\"%s\"}",
				input_ary[0],input_ary[1]);
		
		//cprintf("jasonbuf = %s\n\n\n",jasonbuf);
		snprintf(httprequest, sizeof(httprequest),"POST /WiFi/DEVICE HTTP/1.1\r\nContent-Type:application/json\r\nContent-Length:%d\r\nConnection:Keep-Alive\r\n\r\n%s",strlen(jasonbuf),jasonbuf);
		cprintf("httprequest = %s\n\n\n",httprequest);
		if(connection_with_owl(httprequest, recvline, sizeof(recvline)) < 0)	
			bt_answer(r, "error");
		else
...
extern char *b64_decode( const char* str, unsigned char* space, int size );
void toplogy_cgi(int argc, char **argv, void *p)
{
	CGI_info *ci = (CGI_info *)p;
	struct request_rec *r = ci->r;	
	...
	*/
	...
	else if((!strncmp(cmd, "syncwificonfig", strlen(cmd))) || (!strncmp(cmd, "syncguestwificonfig", strlen(cmd))))
	{
		b64_decode(data, base64buf, sizeof(base64buf));
		cprintf("---> toplogy_cgi, b64_decode data[%s]\n", base64buf);
		for (i = 0;i < strlen(base64buf); i++)
		{
			if (('\"' == base64buf[i]) || ('\\' == base64buf[i]))
			{
				base64buf[i] += 127;
			}
		}		
		snprintf(jasonbuf, sizeof(jasonbuf),"{\"cmd\":\"%s\",\"mac\":\"ALL\",\"data\":\"%s\"}",cmd,base64buf);
	}
	else {
