...
//	cprintf("ssi_dump_FMWLoad_in~~~\n");
	...
	if(strlen(buf) != 0)
	{
		...
		if(strcmp(buf, "err") != 0)
		{
			...
			SetSystemLogFmt(LOG_TYPE_APP, LOG_LEVEL_TYPE_INFO, LOG_APP_MESSAGE_TYPE_http, "HTTP-Find new Firmware %s from TA auto upgrade server.", buf);
#endif
		}
		else
		{
			...
			so_printf(r,"\"available\": '%d'", 0);
#ifdef SYSLOG_ENHANCED
			SetSystemLogFmt(LOG_TYPE_APP, LOG_LEVEL_TYPE_ERROR, LOG_APP_MESSAGE_TYPE_http, "HTTP-Can't connect to TA auto upgrade server.");
#endif
		}
		so_printf(r,"}");
