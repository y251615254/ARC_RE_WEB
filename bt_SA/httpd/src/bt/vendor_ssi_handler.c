...
struct ssi_handler vendor_handlers[] = {
	...
	{ "get_login_info", ssi_get_login_info},	
	{ "get_wifi_psk", ssi_get_wifi_psk},
...
#ifdef WE410443_6DX
//format "0.01.01.01020"
	if( (*(local_version+1) == '.') && (*(local_version+4) == '.') 
		&& (*(remote_version+1) == '.') && (*(remote_version+4) == '.'))
	{
		ret = 1;
	}
	else
		...
		&& (*(remote_version+10) == 'u') && (*(remote_version+11) == 'i') && (*(remote_version+12) == 'l') && (*(remote_version+13) == 'd'))
	{
		...
		{
			if((*(remote_version+16) == 's') || (*(remote_version+16) == 'd') || (*(remote_version+16) == 't') || (*(remote_version+16) == 'v'))
				ret = 1;
			else if(*(remote_version+16) == 'u')
			{
...
//	cprintf("ssi_dump_FMWLoad_in~~~\n");
	...
	if(strlen(buf) != 0)
	{
		...
		if(strcmp(buf, "err") != 0)
		{
			...
			SetSystemLogFmt(LOG_TYPE_APP, LOG_LEVEL_TYPE_INFO, LOG_APP_MESSAGE_TYPE_http, "HTTP-Find new Firmware %s from BT auto upgrade server.", buf);
#endif
		}
		else
		{
			...
			so_printf(r,"\"available\": '%d'", 0);
#ifdef SYSLOG_ENHANCED
			SetSystemLogFmt(LOG_TYPE_APP, LOG_LEVEL_TYPE_ERROR, LOG_APP_MESSAGE_TYPE_http, "HTTP-Can't connect to BT auto upgrade server.");
#endif
		}
		so_printf(r,"}");
...
int ssi_get_access_control_block_mac_info(struct request_rec *r, int argc, char **argv)
{
		...
		if(fifo)
		{
			len = safe_fread(buf, 1, 5120, fifo);	
			so_printf(r,"{\"result\": \"");
			if(len > 0)
			{
				if(buf[len-1] == '\n')
...
int ssi_get_ssid(struct request_rec *r, int argc, char **argv)
{
	...
	char cname[64] = {0};
	char base64ssid[128] = {0};
	int tid;

	if((tid = get_tid()) == MID_FAIL)
		return 0;

	if(strcmp(argv[0], "5hb") == 0)
		sprintf(cname, "ARC_WLAN_5GHB_SSID_%s_ESSID", argv[1]);
	else
	...
	b64_encode(nssid, strlen(nssid), base64ssid, sizeof(base64ssid));
	return so_printf(r, "%s", base64ssid);
}
...
int ssi_get_upgrade_schedule(struct request_rec *r, int argc, char **argv)
{
	...
	mapi_ccfg_get_str(tid, "ARC_FW_Upgrade_Delay_Enable", enable, sizeof(enable));
	mapi_ccfg_get_str(tid, "ARC_FW_Upgrade_Delay_TIME", real_time, sizeof(real_time));
	mapi_ccfg_get_str(tid, "ARC_FW_Upgrade_Time", choose_time, sizeof(choose_time));
	mapi_ccfg_get_str(tid, "ARC_FW_Upgrade_Remote_Version", remote_version, sizeof(remote_version));
	if(atoi(enable) == 1)
	{
		so_printf(r,"{\"enable\": \"%d\",", 1);
		if(strlen(real_time) != 0)
		{
			delayTime = atoi(real_time);
			tm = localtime(&delayTime);
			sprintf(buf, "%02d:%02d", tm->tm_hour, tm->tm_min);
			so_printf(r,"\"real_time\": \"%s\",", buf);
		}
		else
			so_printf(r,"\"real_time\": \"%d\",", 0);
		if(strlen(choose_time) != 0)
			so_printf(r,"\"choose_time\": \"%d\",", atoi(choose_time));
		else
			so_printf(r,"\"choose_time\": \"%d\",", 0);
		if(strlen(remote_version) != 0)
			so_printf(r,"\"remote_version\": \"%s\"", remote_version);
		else
			so_printf(r,"\"remote_version\": \"%s\"", "");		
		so_printf(r,"}");
	}
	else
	{
		so_printf(r,"{\"enable\": \"%d\",", 0);	
		so_printf(r,"\"real_time\": \"%d\",", 0);
		so_printf(r,"\"choose_time\": \"%d\",", 0);
		so_printf(r,"\"remote_version\": \"%s\"", "");
		so_printf(r,"}");		
...
int ssi_get_login_info(struct request_rec *r, int argc, char **argv)
{
	struct sysinfo now_time;
	sysinfo(&now_time);
#if 0	
	pthread_mutex_lock(&login_info.lock);
	cprintf("uptime=%d, error=%d, error_uptime=%d, error_wait_time=%d\n",
		now_time.uptime,  login_info.error, login_info.error_uptime, login_info.error_wait_time);
	
	so_printf(r,"{\"error\": \"%d\",", login_info.error);
	if((login_info.error_wait_time > 0)
		&& (now_time.uptime - login_info.error_uptime < login_info.error_wait_time))
		so_printf(r,"\"error_wait\": \"%d\",", login_info.error_wait_time -now_time.uptime + login_info.error_uptime + 1);
	else
		so_printf(r,"\"error_wait\": \"%d\",", 0);
	
	so_printf(r,"\"error_count\": \"%d\"", login_info.error_count);
	so_printf(r,"}");
	pthread_mutex_unlock(&login_info.lock);
#else
	int tid;
	char tmp_val[32] = {0};
	...
	mapi_tmp_get(tid, "ARC_SYS_LOGIN_ERROR_COUNT", tmp_val, sizeof(tmp_val));
	error_count = atoi(tmp_val);
	memset(tmp_val, 0, sizeof(tmp_val));
	mapi_tmp_get(tid, "ARC_SYS_LOGIN_ERROR_UPTIME", tmp_val, sizeof(tmp_val));
	error_uptime = atoi(tmp_val);
	
	pthread_mutex_lock(&login_info.lock);
	so_printf(r,"{\"error\": \"%d\",", (error_count>0?1:0));
	if((error_count > 4)
		&& (now_time.uptime -error_uptime < ((error_count - 4)*60<900?(error_count - 4)*60:900)))
		so_printf(r,"\"error_wait\": \"%d\",", ((error_count - 4)*60<900?(error_count - 4)*60:900) -now_time.uptime + error_uptime + 1);
	else
		so_printf(r,"\"error_wait\": \"%d\",", 0);
	
	so_printf(r,"\"error_count\": \"%d\"", error_count);
	so_printf(r,"}");
	pthread_mutex_unlock(&login_info.lock);
#endif
	return 0;
...
int ssi_get_wifi_psk(struct request_rec *r, int argc, char **argv)
{
	char opsk[128] = {0};
	char npsk[128] = {0};
	char base64psk[256] = {0};
	char cname[64] = {0};
	int tid;
	...
	if(strcmp(argv[0], "5hb") == 0)
		sprintf(cname, "ARC_WLAN_5GHB_SSID_%s_WPA_Passphrase", argv[1]);
	else
		sprintf(cname, "ARC_WLAN_%sG_SSID_%s_WPA_Passphrase", argv[0], argv[1]);
	
	mapi_ccfg_get_str(tid, cname, opsk, sizeof(opsk));
	checkStringToHTML(opsk, npsk);
	b64_encode(npsk, strlen(npsk), base64psk, sizeof(base64psk));
	return so_printf(r, "%s", base64psk);
}
