...
void set_wificonfig_cgi(int argc, char **argv, void *p);
void do_wps_cgi(int argc, char **argv, void *p);
...
//	{"apply_sipp.cgi",	TASK_t apply_sipp_cgi, 	TASK_STKSIZ},
	...
	{"set_wificonfig.cgi", TASK_t set_wificonfig_cgi, 	TASK_STKSIZ},
	{"do_wps.cgi",	TASK_t do_wps_cgi, 	TASK_STKSIZ},
...
void change_lang_cgi(int argc, char **argv, void *p)
{
	...
	if(lang)
	{
		mapi_ccfg_set_str(tid, "ARC_SYS_Language_Auto", lang);
...
void set_login_info(LOGIN_INFO *info, int fail)
{
	...
	if(fail)
	{
		...
		if(info->error_count > 4)
			info->error_wait_time = ((info->error_count - 4)*60<900?(info->error_count - 4)*60:900);
		else
	...
	memset(tmp_val, 0, sizeof(tmp_val));
	snprintf(tmp_val, sizeof(tmp_val), "%d", error_count);
	mapi_tmp_set(tid, "ARC_SYS_LOGIN_ERROR_COUNT", tmp_val);
	
	memset(tmp_val, 0, sizeof(tmp_val));
	snprintf(tmp_val, sizeof(tmp_val), "%d", error_uptime);
	mapi_tmp_set(tid, "ARC_SYS_LOGIN_ERROR_UPTIME", tmp_val);	
	pthread_mutex_unlock(&info->lock);
#endif
...
void login_cgi(int argc, char **argv, void *p)
{
	...
	{
		get_user_passwd(tid, ARC_SYS_Password2, system_pwd);
		mapi_ccfg_set_str(tid, "ARC_SYS_PrevilegeLevel", "2");
	}
	else
	{
		get_user_passwd(tid, ARC_SYS_Password, system_pwd);
		mapi_ccfg_set_str(tid, "ARC_SYS_PrevilegeLevel", "1");
	}
...
*/
	...
	switch(do_rediret)
	{
		case HTTPD_LOGIN_OK: //0
			...
			//set last connect from GUI msg 
			{
			char  lastip[32];
			if(r->remote_ip.sa.sa_family==AF_INET)
			strcpy(lastip,inet_ntoa(r->remote_ip.sa_in.sin_addr));
#ifdef IPV6_SUPPORT
			else if (r->remote_ip.sa.sa_family==AF_INET6)
			inet_ntop(PF_INET6,&r->remote_ip.sa_in6.sin6_addr,lastip,sizeof(lastip));
#endif
			else 
				strcpy(lastip,"0");
			time_t t;
			...
			mapi_ccfg_set_str(tid, "ARC_SYS_LAST_GUI_Username", name);
			mapi_ccfg_set_str(tid, "ARC_SYS_LAST_GUI_LoginIp", lastip);
			mapi_ccfg_set_str(tid, "ARC_SYS_LAST_GUI_LoginTime", str);
			}
#ifdef SYSLOG_ENHANCED
			login_error_times = 0;
...
int connection_with_owl_get_message(char *request, char *answer, int answerlen)
{
	struct sockaddr_in RemoteAddr;
	int sock_fd;
	int ret = 0;
...
//the fun is for ssid_bt.htm , only used in IE11
void set_wificonfig_cgi(int argc, char **argv, void *p)
{
	CGI_info *ci = (CGI_info *)p;
	struct request_rec *r = ci->r;	
	...
	*/
	...
	if ( !cgi_token_pass(ci, inputs, 0) ) //failed to pass token checking
	{
		so_printf(r, "%s", "cgi_token_pass error");
		//cprintf("[IE11][Debug]-----cgi_token_pass  error  --->[%s_%d] , data[%s]\n", __FUNCTION__,__LINE__, data);
		goto done;//cgi_token Parsing failed only under  IE11, comment out modify by leo
	}
#endif
	/*end pete_zhang	2013-12-20*/
	...
	if(!strncmp(cmd, "syncwificonfig", 14))
	{
		...
		//cprintf("--->[%s_%d] , httprequest[%s]\n", __FUNCTION__,__LINE__, httprequest);
		
		if(connection_with_owl(httprequest, recvline, sizeof(recvline)) < 0)	
			bt_answer(r, "connection_with_owl error");
		else
		{
...
void do_wps_cgi(int argc, char **argv, void *p)
{
	...
	char *sp_begin = NULL;
	char *sp_end = NULL;
	char record_wps_id[20] = {0};

	cprintf("---> [%s %d], enter do wps cgi...\n", __FUNCTION__, __LINE__);
	if((tid = get_tid()) == MID_FAIL)
		return;
	/* Destory last cgi table */
	if ((input_num = __destroy_cgi(inputs, input_num)) != 0)
	...
	*/
	...
	cprintf("---> [%s %d], cmd[%s], data[%s]\n", __FUNCTION__, __LINE__, cmd, data);
	
	if(!strncmp(cmd, "WPS", 3))
	{
		strcpy(mac, data);
		snprintf(jasonbuf, sizeof(jasonbuf),"{\"target\":\"%s\"}", mac);
		
		snprintf(httprequest, sizeof(httprequest),"POST /WiFi/WPS HTTP/1.1\r\nContent-Type:application/json\r\nContent-Length:%d\r\nConnection:Keep-Alive\r\n\r\n%s",strlen(jasonbuf),jasonbuf);

		cprintf("---> [%s %d], mac[%s], httprequest[%s]\n", __FUNCTION__, __LINE__, mac, httprequest);
		if(connection_with_owl_get_message(httprequest, recvline, sizeof(recvline)) < 0)	
			bt_answer(r, "error");
		else
		{
			if(!strncmp(cmd, "WPS", sizeof("WPS")))
			{
				bt_answer(r, "ok_wifi");
			}
			else
		...
		if (sp != NULL)
		{
			...
			{
				sp_end = sp;
				strncpy(record_wps_id, sp_begin, sp_end - sp_begin);
				mapi_tmp_set(tid, "RECORD_WPS_ID", record_wps_id);
			}
		}
	}
