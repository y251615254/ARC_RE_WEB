...
struct ssi_handler vendor_handlers[] = {
	...
	{ "dump_wps_info", ssi_dump_wps_info},
...
int ssi_dump_wps_info(struct request_rec *r, int argc, char **argv)
{	
	char record_wps_id[20] = {0};
	char wps_dir[20] = "/tmp/id_record";
	char cmd[50] = {0};
	FILE *fp = NULL;
	char recvline[20] = {0};
	char result[10] = {0};
	...
	if((tid = get_tid()) == MID_FAIL)
			return 0;
	mapi_tmp_get(tid, "RECORD_WPS_ID", record_wps_id, sizeof(record_wps_id));
	sprintf(cmd, "/tmp/id_record/%s", record_wps_id);
	fp = fopen(cmd, "r");
	if (NULL == fp)
	{
	}
	else
	{
		...
		if (strncmp(recvline, "Success", strlen("Success")) == 0)
			strcpy(result, "Success");
		else
			strcpy(result, "Fail");
		so_printf(r, "{ \"Result\": '");
		so_printf(r, "%s", result);
		so_printf(r, "',\r");
		so_printf(r,"}");
	}
...
int ssi_get_psk(struct request_rec *r, int argc, char **argv)
{
	char opsk[64], npsk[64];
	char cname[64] = {0};
	int tid;
...
#define ALL_DISC_LOG_FILE  "/tmp/All_boosters_log.log"
