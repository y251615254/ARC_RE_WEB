...
void set_cloud_cgi(int argc, char **argv, void *p);
...
//	{"apply_sipp.cgi",	TASK_t apply_sipp_cgi, 	TASK_STKSIZ},
	...
	{"set_cloud.cgi",	TASK_t set_cloud_cgi, 	TASK_STKSIZ},
...
void set_cloud_cgi(int argc, char **argv, void *p)
{
	...
	*/
	...
	if(separate_string_to_array(data, "?$[+", 3, 128, (char *)&input_ary) != 3)
	{
		cprintf("[%s]%d data format error!", __func__,__LINE__);
		return -1;		
	}
...
//	cprintf("[%s_%d]--->\"jasonbuf:%s\n\n\n", __FUNCTION__, __LINE__, jasonbuf);
	...
	if(!strncmp(cmd, "syncsetcloud", 12))
	{
		
		snprintf(jasonbuf, sizeof(jasonbuf),"{\"user_name\":\"%s\",\"server_URL\":\"%s\",\"cloud_enable\":\"%s\"}",input_ary[0],input_ary[1],input_ary[2]);
		//snprintf(jasonbuf, sizeof(jasonbuf),"%s",data);

		snprintf(httprequest, sizeof(httprequest),"POST /tools/Cloud_Server_URL HTTP/1.1\r\nContent-Type:application/json\r\nContent-Length:%d\r\nConnection:Keep-Alive\r\n\r\n%s",strlen(jasonbuf),jasonbuf);
