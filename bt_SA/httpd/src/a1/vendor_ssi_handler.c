...
struct ssi_handler vendor_handlers[] = {
	...
	{ "get_wifi_inface_info", ssi_get_wifi_inface_info},
	{ "get_wifi_radio_info", ssi_get_wifi_radio_info},
	{ "get_wifi_config_info", ssi_get_wifi_config_info},
...
//	cprintf("ssi_dump_FMWLoad_in~~~\n");
	...
	if(strlen(buf) != 0)
	{
		...
		if(strcmp(buf, "err") != 0)
		{
			...
			SetSystemLogFmt(LOG_TYPE_APP, LOG_LEVEL_TYPE_INFO, LOG_APP_MESSAGE_TYPE_http, "HTTP-Find new Firmware %s from A1 auto upgrade server.", buf);
#endif
		}
		else
		{
			...
			so_printf(r,"\"available\": '%d'", 0);
#ifdef SYSLOG_ENHANCED
			SetSystemLogFmt(LOG_TYPE_APP, LOG_LEVEL_TYPE_ERROR, LOG_APP_MESSAGE_TYPE_http, "HTTP-Can't connect to A1 auto upgrade server.");
#endif
		}
		so_printf(r,"}");
...
int ssi_get_wifi_inface_info(struct request_rec *r, int argc, char **argv)
{	
	...
	sprintf(buf_cmd,"GET  /Wifi/Get_Iface_stats HTTP/1.1\r\nContent-Type:application/json\r\nContent-Length:0\r\nConnection:Keep-Alive\r\n\r\n");

	//char *buf ="POST /Wifi/Get_Iface_stats HTTP/1.1\r\nContent-Type:application/json\r\nContent-Length:0\r\nConnection:Keep-Alive\r\nAccept-Encoding:gzip\r\n\r\n";
	...
	if(read(sock_fd, recvline,sizeof(recvline)) > 0)
	{
		...
		p[strlen(p)]='\0';
		//cprintf("[%s_%d]%s", __FUNCTION__, __LINE__, p);
		for(i=strlen(p);i>0;i-=SOPRINTFBUFSIZE)  //data length >  SOPRINTFBUFSIZE
		{	
			memset(soprintfbuff,0,sizeof(char)*(SOPRINTFBUFSIZE+1));
			strncpy(soprintfbuff,p,SOPRINTFBUFSIZE);
...
int ssi_get_wifi_radio_info(struct request_rec *r, int argc, char **argv)
{	
	struct sockaddr_in RemoteAddr;
	int sock_fd,n,i;
	char *p=NULL;
	...
	sprintf(buf_cmd,"GET  /Wifi/Get_Radio_stats HTTP/1.1\r\nContent-Type:application/json\r\nContent-Length:0\r\nConnection:Keep-Alive\r\n\r\n");
...
int ssi_get_wifi_config_info(struct request_rec *r, int argc, char **argv)
{	
	struct sockaddr_in RemoteAddr;
	int sock_fd,n,i;
	char *p=NULL;
	...
	sprintf(buf_cmd,"GET  /Wifi/config HTTP/1.1\r\nContent-Type:application/json\r\nContent-Length:0\r\nConnection:Keep-Alive\r\n\r\n");
...
int ssi_get_upgrade_schedule(struct request_rec *r, int argc, char **argv)
{
	...
	mapi_ccfg_get_str(tid, "ARC_FW_Upgrade_Delay_Enable", buf2, sizeof(buf));
	if(atoi(buf2) == 1)
	{
		mapi_ccfg_get_str(tid, "ARC_FW_Upgrade_Delay_TIME", buf, sizeof(buf));
		if(strlen(buf) != 0)
		{
			delayTime = atoi(buf);
...
int ssi_get_login_info(struct request_rec *r, int argc, char **argv)
{
	...
	if((login_info.error_wait_time > 0)&& (now_time.uptime - login_info.error_uptime < login_info.error_wait_time))
